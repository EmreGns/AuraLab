...
AuraLab yalnızca VB-CABLE routing ile çalışır. Önce VB-Audio VB-CABLE kurun. Uygulama Windows default output'u geçici olarak `CABLE Input (VB-Audio Virtual Cable)` yapar ve `CABLE Output` üzerinden capture eder. Fiziksel cihazlar yalnızca Stereo Routing içindeki sol/sağ output olarak kullanılır; capture kaynağı yapılmaz.
